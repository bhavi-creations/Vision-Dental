...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['slug'])) {
        ...
        // spaces ni hyphen ga convert
        $slug = preg_replace('/\s+/', '-', $slug);

        // special chars remove
        $slug = preg_replace('/[^a-z0-9\-]/', '', $slug);

        // multiple hyphen remove
    ...
    $telugu_title = trim($_POST['telugu_title'] ?? '');

    $telugu_main_content = trim($_POST['telugu_main_content'] ?? '');

    $telugu_full_content = trim($_POST['telugu_full_content'] ?? '');

    $section1_content = trim($_POST['section1_content'] ?? '');

    $section2_content = trim($_POST['section2_content'] ?? '');

    $section3_content = trim($_POST['section3_content'] ?? '');

    $hashtags = trim($_POST['hashtags'] ?? '');

    $keypoints = trim($_POST['keypoints'] ?? '');
